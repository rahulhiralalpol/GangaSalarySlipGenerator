...
export class GeneralService {
  ...
  GeneratePDF(inputExcel: string, outputPDF) {
    ...
    alert("Stage 0 entered function" + inputExcel);
    const workbook: XLSX.WorkBook = XLSX.read(inputExcel, { type: "binary" });
    alert("Stage 1 Workbook read");
    const worksheetname: string = workbook.SheetNames[0];
    alert("Stage 2 Sheets name read");
    const worksheet: XLSX.WorkSheet = workbook.Sheets[worksheetname];
    alert("Stage 3 Sheet read");
    const rows: any = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
    alert("Stage 4 Rows read" + rows);
    ...
    let ypos = 0;
    alert("Stage 2 created writestream pdf");
    ...
    rows.forEach(function (item, index) {
      ...
      doc
        ...
        .text(item.PF_NUMBER, 139.5, 97.169 + ypos, {
          width: 241,
          align: "left"
        }) // EPF No.
        .text(item.EMP_NO, 465.5, 97.169 + ypos, { width: 96.8, align: "left" }) // Employee No.
        .text(item.UAN_NUMBER, 139.5, 111.326 + ypos, {
          width: 241,
          align: "left"
        }) // UAN nO.
